...
├─ models/
│  └─ nuclear_powerplant_low-poly.glb  the 3D model (~1 MB, optimized)
├─ README.md
└─ CREDITS.md                          required attribution (CC BY 4.0)
```

The model was re-encoded with [glTF-Transform](https://gltf-transform.dev/) (Draco
compression, mesh deduplication/joining) to shrink it from the original ~31 MB
download to about 1 MB, with no visible loss of detail — vertex count dropped
less than 3%. Only the storage format changed; the geometry is the same.
...
- The model file is now ~1 MB after optimization (down from the original ~31 MB
  Sketchfab download), so it uploads fine through GitHub's web interface
  (25 MB limit) as well as `git push` (100 MB limit) — no Git LFS needed.
